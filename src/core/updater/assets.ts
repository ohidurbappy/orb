...
/**
 * The release asset name expected for a given platform/arch. Must match the
 * names produced by `scripts/build.ts` and uploaded by the release workflow.
 */
export function assetNameFor(platform: NodeJS.Platform, arch: string): string | null {
  ...
  return os === 'windows' ? `orb-${os}-${cpu}.exe` : `orb-${os}-${cpu}`;
}
